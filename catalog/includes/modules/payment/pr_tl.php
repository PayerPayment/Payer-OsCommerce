...
/*
  $Id: pr_tl.php,v 1.1 2015/04/22 11:57:42 bihla Exp $
...
// class constructor
	function pr_tl() {
		...
		$this->code = 'pr_tl';
		$this->title = MODULE_PAYMENT_PR_TL_TEXT_TITLE;
		$this->description = MODULE_PAYMENT_PR_TL_TEXT_DESCRIPTION;
		$this->sort_order = MODULE_PAYMENT_PR_TL_SORT_ORDER;
		$this->enabled = ((MODULE_PAYMENT_PR_TL_STATUS == 'True') ? true : false);
		$this->name = "Telephone";
		$this->type = "PHONE";

		if ((int) MODULE_PAYMENT_PR_TL_ORDER_STATUS_ID > 0) {
			$this->order_status = MODULE_PAYMENT_PR_TL_NEW_ORDER_STATUS_ID;
		}
...
// class methods
	function update_status() {
		...
		if (($this->enabled == true) && ((int) MODULE_PAYMENT_PR_TL_ZONE > 0)) {
			$check_flag = false;
			$check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_PR_TL_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
	...
	function selection() {
		return array('id' => $this->code,
			'module' => '<img src="/images/payer/p_all.png" width="121" height="41" />');
	}
	...
	function confirmation() {
		...
		if ($insert_order == true) {
			...
			$sql_data_array = array('orders_id' => $insert_id,
				'orders_status_id' => MODULE_PAYMENT_PR_TL_NEW_ORDER_STATUS_ID,
				'date_added' => 'now()',
				'customer_notified' => '0',
				'comments' => "Payer (" . $this->type . "): New order " . (MODULE_PAYMENT_PR_TL_TESTMODE == 'True' ? MODULE_PAYMENT_PR_TL_TEXT_TESTMODE : ""));
	...
	function process_button() {
		...
		$thePostApi->set_test_mode(MODULE_PAYMENT_PR_TL_TESTMODE == "True");
...
// unregister session variables used during checkout
	...
	function get_error() {
		...
		$error = array('title' => MODULE_PAYMENT_PR_TL_TEXT_ERROR,
	...
	function check() {
		if (!isset($this->_check)) {
			$check_query = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_PR_TL_STATUS'");
	...
	function install() {
		...
		tep_db_query("$base values ('Enable Payer Module', 'MODULE_PAYMENT_PR_TL_STATUS', 'True', 'Do you want to accept $this->name payments with Payer?', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', null,now())");

		tep_db_query("$base values ('Enable Payer TestMode', 'MODULE_PAYMENT_PR_TL_TESTMODE', 'True', 'Do you want to use TestMode? (No real $this->name debits will be made)', '6', '1', 'tep_cfg_select_option(array(\'True\', \'False\'), ', null,now())");

		tep_db_query("$base values ('Sort order of display.', 'MODULE_PAYMENT_PR_TL_SORT_ORDER', '4', 'Sort order of display. Lowest is displayed first.', '6', '2' ,null, null, now())");

		tep_db_query("$base values ('Payment Zone', 'MODULE_PAYMENT_PR_TL_ZONE', '0', 'If a zone is selected, only enable this payment method for that zone.', '6', '3', 'tep_cfg_pull_down_zone_classes(', 'tep_get_zone_class_title', now())");

		tep_db_query("$base values ('Set New Order Status', 'MODULE_PAYMENT_PR_TL_NEW_ORDER_STATUS_ID', '0', 'Set new orders to this status (not yet paid)', '6', '4', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");

		tep_db_query("$base values ('Set Payed Order Status', 'MODULE_PAYMENT_PR_TL_ORDER_STATUS_ID', '0', 'Set this status to orders that has been paid', '6', '5', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");
	}
	...
	function keys() {
		return array('MODULE_PAYMENT_PR_TL_STATUS', 'MODULE_PAYMENT_PR_TL_TESTMODE', 'MODULE_PAYMENT_PR_TL_ORDER_STATUS_ID', 'MODULE_PAYMENT_PR_TL_ZONE', 'MODULE_PAYMENT_PR_TL_SORT_ORDER', 'MODULE_PAYMENT_PR_TL_NEW_ORDER_STATUS_ID');
	}
